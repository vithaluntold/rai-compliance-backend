...
async def get_document_status(document_id: str) -> Union[Dict[str, Any], JSONResponse]:
    ...
    try:
        ...
        if os.path.exists(results_path):
            # Read results with better error handling
            try:
                with open(results_path, "r", encoding="utf-8") as f:
                    content = f.read().strip()
                    if not content:
                        logger.warning(f"Empty results file for document: {document_id}")
                        results = {"status": "PROCESSING", "message": "Analysis in progress"}
                    else:
                        # Try to parse JSON, handle multiple objects if necessary
                        try:
                            results = json.loads(content)
                        except json.JSONDecodeError as json_err:
                            logger.error(f"JSON decode error for {document_id}: {json_err}")
                            # Try to salvage partial data by taking only the first complete JSON object
                            lines = content.split('\n')
                            for i, line in enumerate(lines):
                                try:
                                    results = json.loads(line)
                                    logger.info(f"Successfully parsed line {i+1} as JSON for {document_id}")
                                    break
                                except json.JSONDecodeError:
                                    continue
                            else:
                                # If no valid JSON found, return processing status
                                logger.warning(f"No valid JSON found in results file for {document_id}")
                                results = {"status": "PROCESSING", "message": "Analysis in progress"}
            except Exception as file_err:
                logger.error(f"File read error for {document_id}: {file_err}")
                results = {"status": "PROCESSING", "message": "Analysis in progress"}
            # Check for errors
            if "error" in results:
                return {
