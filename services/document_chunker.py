...
class DocumentChunker:
    def __init__(self, min_chunk_length: int = 500, max_chunk_length: int = 4500):
        self.min_chunk_length = min_chunk_length
        self.max_chunk_length = max_chunk_length
        self.dynamic_headers: Set[str] = set()
        logger.info(
            f"Initialized DocumentChunker with min_chunk_length={min_chunk_length}, max_chunk_length={max_chunk_length}"
        )
            ...
            with fitz.open(pdf_path) as doc:
                ...
                for page_num in range(total_pages):
                    ...
                    if len(cleaned_text) >= self.min_chunk_length:
                        # Split oversized chunks to prevent 38KB+ chunks
                        if len(cleaned_text) > self.max_chunk_length:
                            split_chunks = self._split_oversized_text(cleaned_text, page_num)
...
#         )
    ...
    def _split_oversized_text(self, text: str, page_num: int) -> List[Dict[str, Any]]:
        """
        Split oversized text into properly sized chunks to prevent 38KB+ chunks.
        Uses sentence boundaries for clean splits when possible.
        """
        chunks = []
        current_pos = 0
        ...
        while current_pos < len(text):
            ...
            chunk_end = min(current_pos + self.max_chunk_length, len(text))
            ...
            if chunk_end < len(text):
                ...
                # Use the last sentence break if found, otherwise use paragraph break
                if sentence_breaks:
                    chunk_end = sentence_breaks[-1]
                else:
                    # Look for paragraph breaks
                    for i in range(chunk_end - 100, current_pos, -1):
                        if text[i:i+2] == '\n\n':
                            chunk_end = i
                            break
            ...
            if len(chunk_text) >= self.min_chunk_length:
                chunks.append({
                    ...
                    "chunk_type": "content_split",
                })
                logger.info(f"📏 Split oversized text: created chunk of {len(chunk_text)} chars from page {page_num}")
...
# Global instance with proper financial document chunk sizes
document_chunker = DocumentChunker(min_chunk_length=500, max_chunk_length=2000)
